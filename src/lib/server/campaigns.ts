...
const row = (r: Record<string, unknown>): Campaign => ({
  id: r.id as number, channel: r.channel as "email" | "sms", subject: (r.subject as string) ?? null,
...
export async function saveCampaign(c: { channel: "email" | "sms"; subject?: string; body: string; promoCode?: string }): Promise<Campaign> {
  const r = await pool.query(
    `insert into campaigns (channel, subject, body, promo_code) values ($1,$2,$3,$4) returning *`,
    [c.channel, c.subject ?? null, c.body, c.promoCode ?? null],
  );
  return row(r.rows[0]);
}
...
export async function sendCampaign(id: number, origin: string): Promise<Campaign> {
  ...
  if (!klaviyoConfigured() || !newsletterListId()) {
    const error = "Klaviyo not configured (KLAVIYO_API_KEY / KLAVIYO_NEWSLETTER_LIST_ID).";
    await pool.query(`update campaigns set status='failed', error=$2 where id=$1`, [id, error]);
    return { ...c, status: "failed", error };
  }
  ...
  const recipients =
    (await pool.query(`select count(*)::int n from email_subscribers where status='subscribed'`)).rows[0].n as number;

  const res = await sendKlaviyoCampaign({ name: subject || `Campaign ${id}`, subject, html, listId: newsletterListId()!, send: true });
  if (!res.ok) {
