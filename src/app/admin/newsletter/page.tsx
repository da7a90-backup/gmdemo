      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="hero-headline" style={{ fontSize: "clamp(2rem, 4vw, 3rem)", lineHeight: 1.05 }}>
            The <span className="accent-serif">newsletter.</span>
          </h1>
          <p className="mt-3 max-w-2xl text-[15px] text-ink-2 font-serif">
            Write and send the weekly issue. Attach a promotion and the send carries the 2X trigger link;
            opens, clicks, and the subscriber count are tracked per issue.
          </p>
        </div>
        <Label tone="brass" variant="solid">{subs.length} subscribers</Label>
      </div>

      <CampaignDesk channel="email" recipients={subs.length} />

      {/* Subscriber list — secondary, fed by the footer signup */}
      <div className="mt-8">
        <button
          type="button"
          ...
          <ul className="mt-3 border border-ink/10 bg-paper-4 rounded-xl overflow-hidden divide-y divide-ink/10">
            {subs.map((s) => (
              <li key={s.id} className="px-5 py-3 flex flex-wrap items-center justify-between gap-3">
                ...
                >
                  <Trash2 size={11} /> Unsubscribe
                </button>
              </li>
            ))}
            {subs.length === 0 && (
              <li className="px-5 py-6 text-center text-ink-3 font-serif italic">No subscribers yet — the footer signup feeds this list.</li>
            )}
          </ul>
        )}
      </div>
